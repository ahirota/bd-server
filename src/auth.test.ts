...
describe("JWT Functions", () => {
  ...
  it("should fail because of wrong secret", async () => {
    expect(() => validateJWT(validToken, wrongSecret)).toThrowError(new NotAuthorizedError(`Invalid token`));
  });

  it("should fail because of expiration", async () => {
    const fastExp = makeJWT(userID, 0, secret);
    expect(() => validateJWT(fastExp, secret)).toThrowError(new NotAuthorizedError(`Invalid token`));
  });
});